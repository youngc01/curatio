...
async def personalized_manifest(user_key: str, db: Session = Depends(get_db_dependency)):
    """
    ...
    universal_categories = db.query(UniversalCategory).filter(
        UniversalCategory.is_active == True
    ).order_by(
        UniversalCategory.sort_order
    ).all()
    ...
    user_catalogs = db.query(UserCatalog).filter(
        UserCatalog.user_id == user.id,
        UserCatalog.is_active == True
    ).all()
...
async def personalized_catalog(
    ...
    if catalog_id.startswith("universal-"):
        ...
        items = generator.get_catalog_content(actual_id, user_id=user.id)
    else:
...
async def trakt_callback(
    ...
    try:
        ...
        user = db.query(User).filter(
            User.trakt_user_id == str(profile['ids']['slug'])
        ).first()
        ...
        if user:
            ...
            user = User(
                ...
                trakt_token_expires_at=datetime.utcnow() + timedelta(seconds=token_data['expires_in'])
            )
            db.add(user)
