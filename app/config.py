...
class Settings(BaseSettings):
    ...
    universal_catalog_refresh_hours: int = Field(24, description="Hours between universal catalog refreshes")
    personalized_catalog_refresh_hours: int = Field(24, description="Hours between personalized catalog refreshes")
    personalized_catalog_count: int = Field(10, description="Number of personalized catalogs per user")
    ...
    # Feature Flags
    enable_personalized_catalogs: bool = Field(True, description="Enable personalized catalogs")
    enable_universal_catalogs: bool = Field(True, description="Enable universal catalogs")
...
def validate_api_keys():
    ...
    if not settings.trakt_client_id or settings.trakt_client_id == "your_trakt_client_id_here":
        errors.append("TRAKT_CLIENT_ID is not configured")
    
    if not settings.trakt_client_secret or settings.trakt_client_secret == "your_trakt_client_secret_here":
        errors.append("TRAKT_CLIENT_SECRET is not configured")
    
    # Check master password
    if not settings.master_password or settings.master_password == "change_this_to_a_strong_password_123":
...
def get_catalog_url(catalog_type: str, catalog_id: str, user_key: Optional[str] = None) -> str:
    """Generate catalog URL for Stremio."""
    if user_key:
        return f"{settings.base_url}/catalog/{user_key}/{catalog_type}/{catalog_id}.json"
    else:
