...
class UniversalCategory(Base):
    ...
    # Relationships
    catalog_items = relationship("UniversalCatalogContent", back_populates="category", cascade="all, delete-orphan")
...
class UniversalCatalogContent(Base):
    ...
    category_id = Column(String(50), ForeignKey("universal_categories.id", ondelete="CASCADE"), primary_key=True)
    tmdb_id = Column(Integer, primary_key=True)
...
class User(Base):
    ...
    # Relationships
    personal_catalogs = relationship("UserCatalog", back_populates="user", cascade="all, delete-orphan")
...
class UserCatalog(Base):
    ...
    catalog_items = relationship("UserCatalogContent", back_populates="catalog", cascade="all, delete-orphan")
    ...
    def __repr__(self):
        return f"<UserCatalog(id={self.id}, user_id={self.user_id}, name='{self.name}')>"
...
class UserCatalogContent(Base):
    ...
    catalog_id = Column(Integer, ForeignKey("user_catalogs.id", ondelete="CASCADE"), primary_key=True)
    tmdb_id = Column(Integer, primary_key=True)
    ...
    # Indexes
    __table_args__ = (
        Index('idx_catalog_rank', 'catalog_id', 'rank'),
    )
