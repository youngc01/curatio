...
from app.models import Tag, MovieTag, MediaMetadata, TaggingJob
...
PREDEFINED_TAGS = {
    "genre": ["Action", "Crime", "Sci-Fi", "Horror", "Drama", "Comedy", "Thriller", "Romance", "Fantasy", "Mystery", "Documentary", "Animation", "Western", "War"],
    "mood": ["Dark", "Gritty", "Feel-Good", "Quirky", "Suspenseful", "Witty", "Heartwarming", "Intense", "Cerebral", "Lighthearted", "Slow-Burn", "Fast-Paced"],
    "era": ["1920s", "1930s", "1940s", "1950s", "1960s", "1970s", "1980s", "1990s", "2000s", "2010s", "2020s"],
    "region": ["British", "Korean", "French", "Scandinavian", "Japanese", "Italian", "Spanish", "Latin American", "Indian", "Chinese", "American", "European"],
    "plot": ["Heist", "Time Travel", "Revenge", "Courtroom", "Found Footage", "Coming-of-Age", "Survival", "Conspiracy", "Detective", "Ensemble"],
    "style": ["Neo-Noir", "Arthouse", "Independent", "Cyberpunk", "Visually Stunning", "Minimalist", "Period Costume", "Neon Visuals", "Satirical", "Gritty"],
    "character": ["Strong Female Lead", "Anti-Hero", "Ensemble Cast", "Character Study", "Underdog", "Morally Ambiguous", "Reluctant Hero"]
}
...
async def tag_items_batch(db: Session, items: list, media_type: str, batch_size: int = 50):
    """Tag items in batches using Gemini AI."""
    logger.info(f"Tagging {len(items)} {media_type} items in batches of {batch_size}...")
...
async def create_universal_categories(db: Session):
    ...
    categories = [
        ...
        {"id": "dark-crime-dramas", "name": "Dark & Gritty Crime Dramas", "tier": 1, "media_type": "movie", 
         "formula": {"required": ["Dark", "Gritty", "Crime"], "min_required": 2}},
        {"id": "feel-good-comedies", "name": "Feel-Good Comedies", "tier": 1, "media_type": "movie",
         "formula": {"required": ["Feel-Good", "Comedy"], "min_required": 2}},
        {"id": "mind-bending-scifi", "name": "Mind-Bending Sci-Fi Thrillers", "tier": 1, "media_type": "movie",
         "formula": {"required": ["Sci-Fi", "Cerebral", "Suspenseful"], "min_required": 2}},
        {"id": "slow-burn-thrillers", "name": "Slow-Burn Psychological Thrillers", "tier": 1, "media_type": "movie",
         "formula": {"required": ["Slow-Burn", "Thriller", "Cerebral"], "min_required": 2}},
        {"id": "quirky-indie", "name": "Quirky Independent Films", "tier": 1, "media_type": "movie",
    ...
    for cat_data in categories:
        existing = db.query(UniversalCategory).filter(
            UniversalCategory.id == cat_data['id']
        ).first()
...
async def main(movies_limit: int, shows_limit: int):
    ...
    logger.info(f"Estimated time: ~3 hours")
    logger.info("=" * 80)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--movies", type=int, default=100000, help="Number of movies to tag")
    parser.add_argument("--shows", type=int, default=50000, help="Number of TV shows to tag")
